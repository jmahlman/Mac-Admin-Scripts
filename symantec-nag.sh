...
rv=`sudo -u $(ls -l /dev/console | awk '{print $3}') $CocoaDialog msgbox --float --icon-file "$icons/ToolbarInfo.icns" --no-cancel \
...
if [ "$rv" == "1" ]; then
  ...
  exit 0
	...
	if [[ -z $4 ]]; then
		sudo -u $(ls -l /dev/console | awk '{print $3}') $CocoaDialog ok-msgbox --float --no-cancel --icon-file "$icons/ToolbarInfo.icns" \
	  --width "450" --height "150" \
			--title "New Anti-Virus Available from UArts" --text "Please note:" \
	    --informative-text "You will see this alert every day until you install. Thank you."
		else
			sudo -u $(ls -l /dev/console | awk '{print $3}') $CocoaDialog ok-msgbox --float --no-cancel --icon-file "$icons/ToolbarInfo.icns" \
		  --width "450" --height "150" \
				--title "New Anti-Virus Available from UArts" --text "Please note:" \
		    --informative-text "After $install_date, the anti-virus will install automatically. \
     You will see this alert every day until you install. Thank you."
				fi
	if [ "$rv" == "1" ]; then
		exit 0
	fi
